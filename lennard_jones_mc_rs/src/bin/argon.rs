//! アルゴン GEMC テスト計算
//! Chialvo & Horita (2003) 文献条件に準拠

use lennard_jones_mc_rs::{run_gemc, rho_lj_to_g_cm3, GemcParams};
use std::fs::{self, File};
use std::io::Write;

const ARGON_SIGMA_CM: f64 = 3.374e-8;
const ARGON_MOLAR_MASS: f64 = 39.948;

fn main() {
    // Chialvo & Horita (2003): epsilon/k=114.5K, sigma=3.374Å, r_c≈3.0σ
    // 無次元化: epsilon=1, sigma=1, T*=100/114.5≈0.87336, cutoff_radius=3.0
    fs::create_dir_all("results").ok();
    let params = GemcParams {
        temperature: 0.873_362_445_414_847_2, // T* = 100K / 114.5K
        n_total: 500,                         // 文献: N=500
        n_cycles: 30_000,                    // 文献: 3e4 cycles
        delta_max: 0.25,
        delta_v_max: 8.0,
        total_density: 0.30,
        initial_particle_fraction_box1: 0.70,
        initial_volume_fraction_box1: 0.50,
        epsilon: 1.0,
        sigma: 1.0,
        displacement_tries_per_cycle: 500,   // N_total
        transfer_tries_per_cycle: 150,       // 0.3 * N_total
        use_cutoff: true,
        cutoff_radius: Some(3.0),            // 文献: r_c ≈ 3.0 sigma
        use_cell_list: true,
        cell_size: Some(3.0),
        rng_seed: 123,
        progress_interval: 1000,
        sigma_cm: Some(ARGON_SIGMA_CM),
        molar_mass_g: Some(ARGON_MOLAR_MASS),
    };

    println!("=== Argon GEMC (Rust) === ");
    println!("  T* = {:.6} (100 K), N = {}, cycles = {}", params.temperature, params.n_total, params.n_cycles);
    println!("  cutoff = {}, cell_list = {}", params.use_cutoff, params.use_cell_list);
    println!("実行中...\n");

    match run_gemc(&params) {
        Ok(result) => {
            let rho_liq = result.rho1_history.last().unwrap().max(*result.rho2_history.last().unwrap());
            let rho_vap = result.rho1_history.last().unwrap().min(*result.rho2_history.last().unwrap());

            println!("\n最終状態:");
            println!("  Box 1: N={}, V={:.4}, rho* = {:.3} ({:.3} g/cm³), E={:.4}",
                result.box1.n_particles(),
                result.box1.volume(),
                result.box1.density(),
                rho_lj_to_g_cm3(result.box1.density(), ARGON_SIGMA_CM, ARGON_MOLAR_MASS),
                result.box1.energy);
            println!("  Box 2: N={}, V={:.4}, rho* = {:.3} ({:.3} g/cm³), E={:.4}",
                result.box2.n_particles(),
                result.box2.volume(),
                result.box2.density(),
                rho_lj_to_g_cm3(result.box2.density(), ARGON_SIGMA_CM, ARGON_MOLAR_MASS),
                result.box2.energy);
            println!("  rho_liq: rho* = {:.3} ({:.3} g/cm³), rho_vap: rho* = {:.3} ({:.3} g/cm³)",
                rho_liq, rho_lj_to_g_cm3(rho_liq, ARGON_SIGMA_CM, ARGON_MOLAR_MASS),
                rho_vap, rho_lj_to_g_cm3(rho_vap, ARGON_SIGMA_CM, ARGON_MOLAR_MASS));

            let mc = &result.move_counts;
            println!("\n受理率:");
            println!("  Displacement: {:.4}", mc.disp_accept as f64 / mc.disp_total as f64);
            println!("  Volume:       {:.4}", mc.vol_accept as f64 / mc.vol_total as f64);
            println!("  Transfer:     {:.4}", mc.transfer_accept as f64 / mc.transfer_total as f64);

            println!("\n計算時間: {:.3} 秒 ({:.6} sec/cycle)", result.elapsed_secs, result.elapsed_secs / params.n_cycles as f64);

            // CSV 出力
            let csv_path = format!("results/argon_gemc_density_history_N{}_C{}_cutoff_celllist_rs.csv", params.n_total, params.n_cycles);
            if let Ok(mut f) = File::create(&csv_path) {
                let _ = writeln!(f, "cycle,rho1,rho2,rho_liq,rho_vap,n1,n2,v1,v2,e1,e2");
                for i in 0..result.rho1_history.len() {
                    let r1 = result.rho1_history[i];
                    let r2 = result.rho2_history[i];
                    let rl = r1.max(r2);
                    let rv = r1.min(r2);
                    let n1 = result.n1_history[i];
                    let n2 = result.n2_history[i];
                    let v1 = result.v1_history[i];
                    let v2 = result.v2_history[i];
                    let e1 = result.e1_history[i];
                    let e2 = result.e2_history[i];
                    let _ = writeln!(f, "{},{:.16},{:.16},{:.16},{:.16},{},{},{:.16},{:.16},{:.16},{:.16}",
                        i + 1, r1, r2, rl, rv, n1, n2, v1, v2, e1, e2);
                }
                drop(f);
                println!("\n密度履歴を {} に保存しました", csv_path);
            }

            // JSON 風サマリ
            let json_path = format!("results/argon_gemc_summary_N{}_C{}_cutoff_celllist_rs.json", params.n_total, params.n_cycles);
            if let Ok(mut f) = File::create(&json_path) {
                let _ = writeln!(f, "{{");
                let _ = writeln!(f, r#"  "mode": {{ "use_cutoff": true, "cutoff_radius": {}, "use_cell_list": true, "cell_size": {} }},"#, params.cutoff_radius.unwrap_or(0.0), params.cell_size.unwrap_or(0.0));
                let _ = writeln!(f, r#"  "temperature_reduced": {}, "temperature_K": 100.0,"#, params.temperature);
                let _ = writeln!(f, r#"  "N_total": {}, "cycles": {},"#, params.n_total, params.n_cycles);
                let _ = writeln!(f, r#"  "elapsed_s": {},"#, result.elapsed_secs);
                let _ = writeln!(f, r#"  "sec_per_cycle": {},"#, result.elapsed_secs / params.n_cycles as f64);
                let _ = writeln!(f, r#"  "final_box1": {{ "N": {}, "V": {}, "rho": {}, "E": {} }},"#,
                    result.box1.n_particles(), result.box1.volume(), result.box1.density(), result.box1.energy);
                let _ = writeln!(f, r#"  "final_box2": {{ "N": {}, "V": {}, "rho": {}, "E": {} }},"#,
                    result.box2.n_particles(), result.box2.volume(), result.box2.density(), result.box2.energy);
                let _ = writeln!(f, r#"  "final_phase_density": {{ "rho_liq": {}, "rho_vap": {} }},"#,
                    rho_liq, rho_vap);
                let _ = writeln!(f, r#"  "move_acceptance": {{ "displacement": {}, "volume": {}, "transfer": {} }}"#,
                    mc.disp_accept as f64 / mc.disp_total as f64,
                    mc.vol_accept as f64 / mc.vol_total as f64,
                    mc.transfer_accept as f64 / mc.transfer_total as f64);
                let _ = writeln!(f, "}}");
                drop(f);
                println!("サマリを {} に保存しました", json_path);
            }
        }
        Err(e) => println!("エラー: {}", e),
    }
}
